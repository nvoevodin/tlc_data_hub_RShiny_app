...
download2()
    ...
    output$slider <- renderUI({
        sliderInput("year1",label = 'STEP 4: Select Period', min = first(data_m()$year_month), max = last(data_m()$year_month),
    ...
    output$third_main <- renderValueBox({
        ...
        shinydashboard::valueBox(
            ...
            icon = icon("fas fa-taxi"),
            if (box_1st_main()$dif > 0) {color = 'green'} else if (box_1st_main()$dif < 0) {color = 'red'} else {color = 'blue'})
    })
    ...
    output$fourth_main <- renderValueBox({
        shinydashboard::valueBox(if (box_1st_main()$ratio == Inf) {paste0('NO DATA')} else {
            ...
            icon = icon("fas fa-taxi"),
            if (box_1st_main()$ratio > 0) {color = 'green'} else if (box_1st_main()$ratio < 0) {color = 'red'} else {color = 'blue'})
    })
    ...
    output$third_secondary <- renderValueBox({
        ...
        shinydashboard::valueBox(
            ...
            icon = icon("fas fa-taxi"),
            if (box_1st_secondary()$dif > 0) {color = 'green'} else if (box_1st_secondary()$dif < 0) {color = 'red'} else {color = 'blue'})
    })
    ...
    output$fourth_secondary <- renderValueBox({
        shinydashboard::valueBox(if (box_1st_secondary()$ratio == Inf) {paste0('NO DATA')} else {
            ...
            icon = icon("fas fa-taxi"),
            if (box_1st_secondary()$ratio > 0) {color = 'green'} else if (box_1st_secondary()$ratio < 0) {color = 'red'} else {color = 'blue'})
    })    
    ...
    output$hvsharing = renderValueBox({ 
        ...
        print(shared)
        valueBox(
            if (is.na(head(shared,1)) == T) {paste0('No Data')} else {
                paste0(shared)}, 
            if (is.na(head(shared,1)) == T) {paste0('.')} else {
                paste0('% of FHV - High Volume trips per day that are shared as of ', as.yearmon(input$monthdate[2]))}, icon = icon("fas fa-taxi"),
            color = "aqua")
    })
